...
//A JSON FILE THAT IS TOO BIG FOR PHP TO HANDLE.
	...
	function createCanvas(targetDiv, id, size){	
		this.div = targetDiv;
		this.id = id;
		this.size = size;
		this.canvas = this.makeCanvas(targetDiv, id, size);
		this.set_opacity = function(){

		}
		
		this.makeCanvas = function(targetDiv, id, size){
				var canvas = d3.select("div#" + targetDiv)
							.append("svg:svg")
							.attr("id",id)
	...
	function selectAlternateView(){
		if (document.getElementById("mainCanvasSelector").checked){
			...
			d3.select("#NetworkView").style("display", "none");
			...
			document.getElementById("pvalueContainer").style.display="none"
			d3.select("#NetworkView").style("display", "inline");
			document.getElementById("pvalueSVG").style.display="none";
	...
	function getJSON(i, canvasRGB, indicatorColor){
		...
		// Gets the canvas JSON file from server synchronously. 
			...
			xmlhttp.open("GET","getJSON_mySQL.php?number="+number, false);
			xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			xmlhttp.send();			
	...
	//-------------------------------------
		...
		function fill(nodes, elements, RGB, width){
			...
			manOutput.push(clusterFind(manhattanNodes, width));
					...
					manhattan.attr("title", "Negative z-score corresponds to clustering.");
					...
					geneTable.selectAll("th").data(["Avg. Distance", "Standard Dev.", "Z-Score"]).enter()
	...
	function clusterFind(nodes, width){
			...
			// Note that "nodes" are the selected nodes. <-----IMPORTANT
			// 
			...
			var x1, x2, x2, y2, 
				totalSum, distance, nearestDist, chosen, i,
				avgNN, m, z
			...
			for (chosen = 0; chosen < nodes.length; chosen++){
				nearestDist = 9999;
			...
			// Create contigency table for Fisher Test
				...
				for (var key in infos){
					...
					if (contA !== 0){
						...
						nodeList.push([key.toUpperCase(), pvalue, pvalue, pvalue, contC, elementList.length, contA, genesIntersect.join(";")])
				...
				nodeList.sort(function(a,b){return b[1]-a[1]}); //Reverse for Benjamini-Hochberg
				
				var nodelistCount = nodeList.length;
				var previousValue = 1;
				var adjustedPvalue = 0;

				for (var i = 0, maxLenNodeList = nodeList.length; i < maxLenNodeList; i++){
					adjustedPvalue = nodeList[i][1] *  maxLenNodeList / nodelistCount; 
					
					nodeList[i][1] = (adjustedPvalue < previousValue) ? adjustedPvalue : previousValue; //Preserve Monotonicity
					nodeList[i][2] = nodeList[i][3] * nodeList.length;
					previousValue = nodeList[i][1]
					nodelistCount -= 1;
				}
...
//hacking Chris's code to get original p-values without correction
					nodeList[i][1] = nodeList[i][3].toExponential(3);
					nodeList[i][2] = nodeList[i][3].toExponential(3);
					nodeList[i][3] = nodeList[i][3].toExponential(3);
					if (i < 20){
			...
			var enWidth = d3.select("#enrichmentResults").style("width")

			baseTable = GSE.append("table").attr("id", "GSEElement1").attr("class", "contain");
			baseTable.selectAll("th").data(["Node", "P-value", "P-value", "Pvalue (Original)"]).enter().append("th").text(function(d){return d;});
			for (var i = 0; i < nodeList.length; i++){
				var tableRow = baseTable.append("tr");
				for (var x = 0; x < 4; x++){
					tableRow.append("td")
						.text(nodeList[i][x])
						.style("width", enWidth/2)
						.attr("title", nodeList[i][0]);
				}		
			}
			...
			// Adding Manhattan 
					...
					var geneTable = baseTable.append("td").attr("id", "col2").append("table").attr("id", "manhattan_gene")
					geneTable.selectAll("th").data(["Avg. Distance", "Standard Dev.", "Z-Score"]).enter()
						.append("th").text(function(d){return d;});
