...
//A JSON FILE THAT IS TOO BIG FOR PHP TO HANDLE.
	...
	function pvalueCanvas(nodeList){
		...
		function gameFind(){
			...
			if (cI > -1){
				points +=  (20 - cI) * Math.floor((1 + (10 - cI)/100) * (1 + 0.001 * G_VAR.nodes.length) * 10/ ( 1 + 0.01 * timeGame));
				persistCount += 1;
				/*
				var oriNum = []
					for (var x=0; x<3; x++){
				...
				*/
				...
				var str = ["rgb(", G_VAR.indicatorColor.join(","), ")"].join("");
					
				nodeI = G_VAR.reverseDict[nN];
				//for (var a = 0; a < nodeI.length; a++){
					d3.selectAll(".circles#" + persistentNodeNames[cI])
						.attr("opacity", 1)
						.attr("fill", str);
					d3.selectAll(".circles#" + persistentNodeNames[cI])
						.append("title")
	...
	function getGMT(i){
			...
			xmlhttp2.open("GET","getGMT.php?number=" + number, true);
	...
	//------------------------------
		...
		function calculateGeneFill(nodes, elements, hexCode, infoDict, infos){
			...
			for (var i = 0; i < dispNodes.length; i++){
				nodes[dispNodes[i][0]].circleMaker(hexCode, 1);
			}
	...
	//------------------------------
		...
		function circleMake(nodes, pixels){
			...
			var radius = Math.floor(pixels/2.75);
		...
		function rectMake(nodes, pixels){
			...
			for (var i = 0; i < nodes.length; i++){
				...
				rects.attr("x", function(d){ return d.index%width * pixels;})
							...
							.attr("id", function(d){ return d.searchText;});
				rects.append("title")
			        .text(function(d) { return d.text; });
			}
		}		
