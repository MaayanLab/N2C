...
//A JSON FILE THAT IS TOO BIG FOR PHP TO HANDLE.
	...
	function loadPatients(){
		patients = [];
		d3.json("../GMT/patientGMT_lvl3_dict.json", function(json){ patients = json})
		patientArray = []
		document.getElementById("selectPatient").disabled = true;
		var waitPatients = self.setInterval(function(){
				
				if (isEmpty(patients) == true){
				}
				else {
					clearInterval(waitPatients);
					for (var patient in patients){
						patientArray.push(patient);			
					}
					
					patientArray.sort();
					//d3.select("#selectPatient").selectAll("option").data(patientArray).append("option").attr("value", function(d){return d;}).text(function(d){return d;});
					var select = document.getElementById("selectPatient");
					var num = 1;
					var len = patientArray.length;
					
					
					for (var index = 1; index < len; index++){
						select.options[index] = new Option(patientArray[index], patientArray[index]);
					}
					document.getElementById("selectPatient").selectedIndex = 0;
					document.getElementById("selectPatient").disabled = false;
	...
	function getScoreBoard(){
		...
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200){
			...
			scoreboard.append("tr").selectAll("th").data(["Rank", "Name", "Score"])
	...
	function modifyScoreBoard(score, name){
		...
		/*
		if (showScore[9][2] > score)
		{
			window.alert("Sorry, your score does not make the top 10 high scores.");
			return;
		}
		*/
		...
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200){
			...
			window.alert("Your rank is" + json[0]);
			getScoreBoard();
		}
	...
	function getPatient(selectedIndex){
		
		document.getElementById("genes").value = patients[document.getElementById("selectPatient").options[selectedIndex].value].join('\n');
	
	}
	
	function randomPatient(){
	
		document.getElementById("holdStartButton").innerHTML= "";
		var roughDuration = 4000;
		var len = document.getElementById("selectPatient").options.length;
		document.getElementById("submitButton").disabled = true;
		document.getElementById("clearButton").disabled = true;
		document.getElementById("randButton").disabled = true;
		document.getElementById("selectPatient").disabled = true;
		
		sel_Patient = document.getElementById("selectPatient");
		var randomWait = self.setInterval(function(){
			
			var sI_patient = Math.floor(Math.random() * len + 1);
			sel_Patient.selectedIndex = sI_patient;
			
			
			if (roughDuration <= 0){
				clearInterval(randomWait);
				document.getElementById("genes").value = patients[sel_Patient.options[sI_patient].value].join('\n');
	...
	function pvalueCanvas(nodeList){
		...
		function gameStart(){
			d3.select("#pvalueSVG").remove();
			d3.select("#startButton").remove();
			nodeList = G_VAR.nodeList;
			pvalueSVG = d3.select("#pvalueContainer")
								.append("svg:svg")
								.attr("id", "pvalueSVG")
								.attr("width", 275)
			...
			var indicate = G_VAR.indicatorColor;
			var width = Math.sqrt(G_VAR.nodes.length);
			var maxDistance = maxDistance(width);
			counterPersistent = 0;
			var pixels = 275 / width; 
			for (var i = 0; i < nodeList.length; i++) { 
				nodeDict[nodeList[i][0].toUpperCase()] = parseFloat(nodeList[i][1]);
		...
		function gameFind(){
			...
			var nodeI = G_VAR.reverseDict(nN);
			
			if (nN == persistentNodeNames[counterPersistent])
			{
				for  (var a = 0; a < nodeI.length; a++)
				{
					d3.select("#pCanvas" + nodeI[a]).attr("fill", "#FFFFFF");
			...
			{	
				...
				var str = ["rgb(", ((maxDistance - closestDistance) * 255 /maxDistance), 0 , (255 * closestDistance / maxDistance) ")"].join("");
				
				d3.select("#pCanvas" + index).attr("fill", str);
				d3.select("#pCanvas" + index).append("title", nodeName);
			...
			/*
			if (cI > -1){
				points +=  (20 - cI) * Math.floor((1 + (10 - cI)/100) * (1 + 0.0001 * G_VAR.nodes.length) * 10/ ( 1 + 0.01 * timeGame));
				persistCount += 1;
				var oriNum = []
					for (var x=0; x<3; x++){
						if (min !== max){
							oriNum.push(Math.floor(indicate[x] * (0.4 + 0.6 * (1 - (persistentNodeDict[nN] - min) / (max - min))), 4));

						}
						else {
							oriNum.push(G_VAR.indicatorColor[x]);
						}
						
					}
					console.log(oriNum);
				var str = ["rgb(", oriNum.join(","), ")"].join("");
					
				nodeI = G_VAR.reverseDict[nN];
				for (var a = 0; a < nodeI.length; a++){
					d3.select("#pCanvas" + nodeI[a])
						.attr("fill", str);
					d3.select("#pCanvas" + nodeI[a])
						.append("title")
						.text(persistentNodeNames[cI]);
			
				}
				d3.selectAll("#pvalueRow" + cI).style("color", "rgb(0,127,0)");
				persistentNodeNames[cI] = "";
			...
			*/
			...
			if ((persistCount == maxNodes) || (persistCount == 10)){
				goodGame = true;
			}
			...
			if (clicksLeft > 0 && persistCount < maxNodes && goodGame == false){
				document.getElementById("timer").innerHTML = "<table style = 'width:390px'><tr><td>Points: " + points + "</td><td> Nodes Chosen: " + (persistCount) + " / 10 </td></tr><tr><td>Time: " + timeGame + "</td><td>Clicks Left: " + clicksLeft + "</td></tr></table>";
			}
		...
		function timer(){
			...
			if (goodGame == true){
				...
				document.getElementById("selectionDisplay3").innerHTML = '<input type="button" name="button" id = "scoreBoardButton" value=points onclick="scoreBoardMod(points);" />'
				}
	...
	function buildNetwork(links, w, indicatorColor){
		var nodes ={};
			...
			document.getElementById("networkView").disableddisabled = false;	
	...
	//------------------------------
		...
		function calculateGeneFill(nodes, elements, hexCode, infoDict, infos){
			...
			for (var i = 0; i < nodeList.length; i++){
				var tableRow = baseTable.append("tr").attr("id", "pvalueRow" + i).style("color", "#FF0000");
				for (var x = 0; x < 2; x++){
			...
			//Builds the Network View
				...
				d3.selectAll("#NetworkView").remove();
					...
					selectAlternateView();
					document.getElementById("holdStartButton").innerHTML="<input type='button' id='startButton' value = 'Start Game!' onClick = 'gameStart()'></input>"
	}
