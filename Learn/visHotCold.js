...
//A JSON FILE THAT IS TOO BIG FOR PHP TO HANDLE.
	...
	function pvalueCanvas(nodeList){
		...
		function gameStart(){
			...
			document.getElementById("selectionDisplay3").innerHTML = "<br /> <br />Please Find : " + persistentNodeNames[counterPersistent];
			var min = nodeList[0][1]
			var max = nodeList[nodeList.length - 1][1]
			var nodeNames = [];
			var nodeDict = {};
		...
		function timer(){
			...
			if (goodGame == true){
				clearInterval(myVar);
				/*
				document.getElementById("holdStartButton").innerHTML="<input type='button' id='startButton' value = 'Continue!' onClick = 'gameContinue()'></input>"*/
