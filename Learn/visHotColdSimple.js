...
//A JSON FILE THAT IS TOO BIG FOR PHP TO HANDLE.
	...
	{	
		...
		document.getElementById("holdStartButton").innerHTML="<input type='button' id='startButton' value = 'Start Game/Timer!' onClick = 'gameStart()'></input>"
		d3.select
	}
	...
	function pvalueCanvas(nodeList){
		...
		function gameFind(){
			...
			if (clicksLeft > 0 && counterPersistent < maxNodes && goodGame == false){
				document.getElementById("timer").innerHTML = "<table style = 'width:390px'><tr><td>Points: " + points + "</td><td> Nodes Chosen: " + (counterPersistent + 1) + " / 20 </td></tr><tr><td>Time: " + timeGame + "</td><td>Clicks Left: " + clicksLeft + "</td></tr></table>";
			}
		...
		if (!(counterPersistent >= 20) && !(clicksLeft <= 0))
		{
			goodGame = false;
			gameStart();
		} else if ((counterPersistent >= 20)){
			clearInterval(myVar);
		...
		function calculateGeneFill(nodes, elements, hexCode, infoDict, infos){
			...
			//Builds the Network View
				...
				d3.selectAll("#NetworkView").remove();
					...
					selectAlternateView();
					document.getElementById("holdStartButton").innerHTML="<input type='button' id='startButton' value = 'Start Timer!' onClick = 'gameStart()'></input>"
	}
